...
const AdminDashboard = () => {
  ...
  const stats = [
    ...
    { id: 3, label: "Appointments", value: 312 },
  ];
      ...
      <div className="flex items-start">
            ...
            <div class="py-6 px-6">
              ...
              {customMenus.length > 0 && (
                <ul class="space-y-2 mt-4">
                  {customMenus.map((m, i) => (
                    <li key={i}>
                      <a href="#" className="menu-item text-slate-800 text-[15px] font-medium flex items-center cursor-pointer hover:bg-[#F0F8FF] rounded-md px-3 py-3 transition-all duration-300">
                        <span>{m}</span>
                      </a>
                    </li>
                  ))}
              ...
              <div class="mt-8">
                <div class="bg-[#00b074] p-4 rounded-md shadow-md max-w-[196px]">
                  ...
                  <button type="button"
                    ...
                    className="py-2 px-4 bg-white hover:bg-gray-100 text-slate-800 text-sm border-0 outline-0 rounded-md cursor-pointer mt-4">Add
                    Menu</button>
                </div>
              </div>
            </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <form onSubmit={addMenu} className="relative bg-white rounded p-6 w-full max-w-md z-10">
              <h3 className="text-lg font-medium mb-3">Create Menu</h3>
              <input value={newMenuLabel} onChange={(e)=>setNewMenuLabel(e.target.value)} placeholder="Menu label" className="w-full border p-2 rounded mb-3" />
       ...
       <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 ">
          {stats.map((s) => (
           <div key={s.id} className="bg-white p-4 rounded shadow [box-shadow:0_4px_12px_-5px_rgba(0,0,0,0.4)] p-6 w-full max-w-sm rounded-lg overflow-hidden">
             <div className="text-sm text-gray-500">{s.label}</div>
             <div className="text-2xl font-bold">{s.value}</div>
           </div>
         ))}
       ...
       <div className="bg-white p-4 rounded shadow [box-shadow:0_4px_12px_-5px_rgba(0,0,0,0.4)] rounded-lg overflow-hidden">
         <h2 className="text-lg font-medium mb-3">Recent Activity</h2>
         <p className="text-sm text-gray-600">No recent activity to show.</p>
       </div>
     </div>
